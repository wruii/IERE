...
sam_config = {'sam':{'image_size':256, 'model_type':'vit_b', 'sam_checkpoint':"../../SSL-MedSeg-main/SAM-Med2D-main/workdir/models/sam-med2d-ba-26/epoch22_sam.pth",
...
class CardiacSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.config["model"]["weights"]:
            pretrained_weights = get_state_dict_form_pretrained_model_label("prostate_b_to_a.ckpt")
    ...
    def setup(self, stage=0):
        if self.config["dataset"].lower() == "prostate":
            self.train_dataset = TrainProstateDataset(self.config["dataset_root"], "train", self.config["split_file"])
            self.val_dataset = TrainProstateDataset(self.config["dataset_root"], "val", self.config["split_file"])
            self.test_dataset = TrainProstateDataset(self.config["dataset_root"], "test", self.config["split_file"])
    ...
    def training_step(self, batch, batch_idx):
        ...
        mixl_img = img_a * img_mask + unimg_a * (1 - img_mask) # label primary
...
def main():
    ...
    default_config_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'config_sam_prostate_cp.yaml')
